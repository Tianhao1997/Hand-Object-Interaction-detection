...
class roibatchLoader(data.Dataset):
    ...
    def __init__(self, roidb, ratio_list, ratio_index, batch_size, num_classes, training=True, normalize=None):
        ...
        :param ratio_list: 1D array [ratio1, ratio2,...], ratio order of all images from small to large
        :param ratio_index: 1D array [28854, 28853, 13412 ... 59868] shows the original index order before sorting
        :param batch_size: an int, (1 / 2 / 4 / 8 ...)
        :param num_classes: 3 ('__background__', 'targetobject', 'hand')
        """
        self._roidb = roidb
        ...
        for i in range(num_batch):
            ...
            if ratio_list[right_idx] < 1:
                # for width < height, we preserve the leftmost in each batch.
                target_ratio = ratio_list[left_idx]
            elif ratio_list[left_idx] > 1:
                # for width > height, we preserve the rightmost in each batch.
    ...
    def __getitem__(self, index):
        ...
        if self.training:
            ...
            blobs['gt_boxes'], blobs['box_info'] = unison_shuffled_copies(blobs['gt_boxes'], blobs['box_info'])
            ...
            box_info_padding = torch.FloatTensor(self.max_num_box, box_info.size(1)).zero_()
            if keep.numel() != 0:
